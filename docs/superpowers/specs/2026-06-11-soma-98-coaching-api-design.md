...
User evaluation of the AI result is stored by a separate follow-up API after the user reviews the result.
...
After reviewing a completed coaching result, the user can submit one lightweight evaluation.

```http
POST /api/v1/coachings/{coachingId}/evaluation
...
| `comment` | string | no | Optional user opinion. Empty strings should be stored as `null`. |
...
- Each coaching can have at most one evaluation in the alpha implementation.
- Missing or out-of-range `rating` returns `400 Bad Request`.
...
Use MySQL for SOMA-98 persistence. Manage schema changes with Flyway migrations, not runtime schema generation. Use one `coachings` table for the alpha implementation. Store the uploaded video metadata, request context, AI execution metadata, normalized `CoachFeedback` fields, and failure details in the same row. Do not store the raw video bytes in the database.
...
- `V1__create_coachings.sql`
- `V2__create_coaching_evaluations.sql`
...
Use MySQL `json` for `result_focus_axes`.

Use a separate `coaching_evaluations` table for user evaluation:
...
- Duplicate evaluation for the same coaching returns `409 Conflict`
