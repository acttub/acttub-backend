...
Use one `coachings` table for the alpha implementation. Store the uploaded video metadata, request context, AI execution metadata, normalized `CoachFeedback` fields, and failure details in the same row. Do not store the raw video bytes in the database.
...
When the database is selected, use the closest native JSON column type for `result_focus_axes` such as `jsonb` on PostgreSQL.
